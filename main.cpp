...
int main()
{
    ...
    double qj1=10000;
    double qj2=6000;
    double q_=Wj1*(Tjav1-Tj01)+Wj2*(Tjav2-Tj02);//Потребная теплота
    double Tav=(Wi1*Ti01+Wi2*Ti02-q_)/(Wi1+Wi2);//Температура горячих потоков на выходе
    double sigma0=Wi1*(log(Tav/Ti01))+Wi2*(log(Tav/Ti02))
    +Wj1*(log(Tjav1/Tj01))+Wj2*(log(Tjav2/Tj02));//Производство энтропии
    double W2=Wi1+Wi2;
    //Найдем величины коэффициентов теплообмена
    //Первый интервал
